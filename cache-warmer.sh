#!/bin/bash
# Cache Warmer Script v4.9 (Final Production Version)
set -euo pipefail
...
if [[ $# -lt 2 ]]; then echo "Usage: $0 <domain> <sitemap_path> [verbose]" >&2; exit 1; fi

URL_DOMAIN=$1; SITEMAP_PATH=$2; VERBOSE_FLAG="${3:-}"
SITEMAP_INDEX_URL="https://$URL_DOMAIN/$SITEMAP_PATH"; LOCK_FILE="${LOCK_DIR}/${URL_DOMAIN}.lck"

log() { echo "[$(date +'%Y-%m-%d %H:%M:%S')] [$URL_DOMAIN] $1" >&2; }

# Self-locking using flock on a file descriptor. This is the most robust method.
exec 200>"$LOCK_FILE"
if ! flock -n 200; then
...
warm_url() {
    ...
    desktop_time=$(nice -n 19 curl --max-time 30 -A "$USER_AGENT_DESKTOP" -sL --compressed -w "%{time_total}" "$url_to_warm" -o /dev/null || echo "0")
    mobile_time=$(nice -n 19 curl --max-time 30 -A "$USER_AGENT_MOBILE" -sL --compressed -w "%{time_total}" "$url_to_warm" -o /dev/null || echo "0")
...
main() {
    ...
    echo "$timestamp,$URL_DOMAIN,$total_unique_count,${#final_urls[@]},$desktop_stats,$mobile_stats" >> "$STATS_FILE"
